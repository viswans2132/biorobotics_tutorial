...
class JointCommander(Node):
    def __init__(self, namespace=''):
        ...
        self._joint_pub = self.create_publisher(Twist, '/unitree1/cmd_vel', qos_profile)
        ...
        self._cur_pos = np.array([1.3, -2.0])
        self._yaw = 0.0
        self._des_pos = np.array([-2.0, -2.0])

        self._safety_radius = 1.0

        self._obs_pos = np.array([2.5, 1.0])
        # self._obs_pos = np.array([-2.0, -2.0])
        self._opt_mng = og.tcp.OptimizerTcpManager('bd/quad_mpc')
        self._opt_mng.start()

    def mpc_cost(self, u):
        cost = 0.0
        updated_pos = self._cur_pos
        u = u.reshape(self._horizon,-1)
        q = 1.0
        r = 5.0
        for i in range(self._horizon):
            upPosErr = self._des_pos - updated_pos
            cost = cost + q*(upPosErr@upPosErr) + r*(u[i]@u[i])

            updated_pos = updated_pos + u[i]*self._dt
        return cost

    def cons_value(self, u):
        updated_pos = self._cur_pos
        u = u.reshape(self._horizon, -1)
        for i in range(self._horizon):
            obsDist = la.norm(self._obs_pos - updated_pos)
            if obsDist < 1.0:
                return obsDist - 1.0
            updated_pos = updated_pos + u[i]*self._dt
        return 0
    ...
    def cmdloop_callback(self):
        ...
        if la.norm(posErr) < 0.1:
            ...
            if la.norm(posErr) > 0.5:
                ...
                goal_msg.angular.z = 0.1*yawErr



            # Add the joint angles to the goal message
            states = np.array([self._cur_pos[0], self._cur_pos[1], self._des_pos[0], self._des_pos[1],
            					self._obs_pos[0], self._obs_pos[1], self._safety_radius])
            nominal_values = [0.1*(self._des_pos[0] - self._cur_pos[0]), 0.1*(self._des_pos[1] - self._cur_pos[1])]*20
            sol = self._opt_mng.call(states, initial_guess=nominal_values, buffer_len=8*4096)
            ...
            # # print(posErr)
            # print(f'Command: {posErr[0]:.2f}: {posErr[1]:.2f}')
            goal_msg.linear.x = np.minimum(np.maximum(u_body[0], -0.3), 0.3)
            goal_msg.linear.y = np.minimum(np.maximum(u_body[1], -0.3), 0.3)
