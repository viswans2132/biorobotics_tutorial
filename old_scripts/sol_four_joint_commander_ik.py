...
class JointCommander(Node):
    def __init__(self, namespace=''):
        ...
        self._link_len = np.array([0.25, 0.25])
    ...
    def cmdloop_callback(self):
        ...
        setpoint1 = np.array([0.0, 0.1])
        setpoint2 = np.array([0.0, 0.2])
        ...
        x1 = self._link_len[0]*np.cos(ik1[0]) + self._link_len[1]*np.cos(ik1[0] + ik1[1])
        y1 = self._link_len[0]*np.sin(ik1[0]) + self._link_len[1]*np.sin(ik1[0] + ik1[1])
    ...
    def inverseKinematics(self, pos):
        ...
        c2 = (pos@pos - self._link_len@self._link_len)/(2*self._link_len[0]*self._link_len[1])
        s2 = np.sqrt(1 - c2*c2)
        theta2 = np.atan2(s2,c2) 

        k1 = self._link_len[0] + self._link_len[1]*c2;
        k2 = self._link_len[1]*s2;
        theta1 = np.atan2(pos[1],pos[0]) - np.atan2(k2,k1);
