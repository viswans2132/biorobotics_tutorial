///
/// Auto-generated TCP server for optimizer: quad_mpc
///
...
fn execution_handler(
    ...
    match initial_guess {
        ...
        Some(u0) => {
            if u0.len() != QUAD_MPC_NUM_DECISION_VARIABLES {
    ...
    if parameter.len() != QUAD_MPC_NUM_PARAMETERS {
...
fn main() {
    let matches = App::new("OpEn TCP Server [quad_mpc]")
        .version("0.0.0")
        .author("John Smith")
