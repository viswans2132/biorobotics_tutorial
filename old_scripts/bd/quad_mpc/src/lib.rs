...
pub const QUAD_MPC_NUM_DECISION_VARIABLES: usize = 40;

/// Number of parameters
pub const QUAD_MPC_NUM_PARAMETERS: usize = 7;

/// Number of parameters associated with augmented Lagrangian
pub const QUAD_MPC_N1: usize = 0;

/// Number of penalty constraints
pub const QUAD_MPC_N2: usize = 20;
...
const CONSTRAINTS_XMIN :Option<&[f64]> = Some(&[-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,-0.6,]);
const CONSTRAINTS_XMAX :Option<&[f64]> = Some(&[0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,0.6,]);
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(QUAD_MPC_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, QUAD_MPC_N1, QUAD_MPC_N2)
}
...
fn unscale_result(solver_status: &mut Result<AlmOptimizerStatus, SolverError>) {
    if let Ok(sss) = solver_status {
        let w_cost : f64 = icasadi_quad_mpc::get_w_cost();
...
pub fn solve(
    ...
    assert_eq!(p.len(), QUAD_MPC_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), QUAD_MPC_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    // Start by initialising the optimiser interface (e.g., set w=1)
    icasadi_quad_mpc::init_quad_mpc();
    ...
    if DO_PRECONDITIONING {
        ...
        icasadi_quad_mpc::precondition(u, p);

        // Compute initial penalty
        icasadi_quad_mpc::initial_penalty(u, p, & mut rho_init);
    }

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_quad_mpc::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_quad_mpc::grad(u, xi, p, grad);
        Ok(())
    };
    
    let f2 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_quad_mpc::mapping_f2(u, p, res);
        Ok(())
