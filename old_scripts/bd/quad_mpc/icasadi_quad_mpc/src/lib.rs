...
const NUM_STATIC_PARAMETERS: usize = 7;
...
const NUM_CONSTRAINTS_TYPE_PENALTY: usize = 20;

use libc::{c_double, c_int};  // might need to include: c_longlong, c_void

// C interface (Function API exactly as provided by CasADi)
extern "C" {
    fn init_interface_quad_mpc();
    fn cost_function_quad_mpc(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double) -> c_int;
    fn grad_cost_function_quad_mpc(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double)
        -> c_int;
    fn mapping_f1_function_quad_mpc(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double,
    ) -> c_int;
    fn mapping_f2_function_quad_mpc(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double,
    ) -> c_int;
    // Compute all preconditioning parameters
    fn preconditioning_www_quad_mpc (
        arg: *const *const c_double,
    ) -> c_int;
    fn init_penalty_function_quad_mpc(
        arg: *const *const c_double,
        ipnlt: *mut *mut c_double,
    ) -> c_int;
    fn get_w_cost_quad_mpc() -> c_double;
} // END of extern C
...
pub fn precondition(
    ...
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    unsafe {
         preconditioning_www_quad_mpc(arguments.as_ptr()) as i32
    }
}

/// Computes the initial penalty
///
/// Make sure that you have called init_quad_mpc
pub fn initial_penalty(
    u: &[f64],
    static_params: &[f64],
    rho_init: &mut f64,) -> i32 {

    // preconditioning_init_penalty_function_quad_mpc
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    let ip = &mut [rho_init as *mut c_double];

    unsafe {
         init_penalty_function_quad_mpc(
            arguments.as_ptr(),
            ip.as_mut_ptr(),
        ) as i32
    }
